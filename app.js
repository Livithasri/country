...
const putData = (finalData) => {
    ...
    <div class="card mx-3 my-3" style="width: 18rem;">
        ...
        <div class="card-body">
            ...
            <p class="card-text">Sub-Continent: ${country.subregion}</p>
            <p class="card-text">Population: ${country.population}</p>
            <p class="card-text">Language: ${country.languages[0].name}</p>
...
document.querySelector('#search').addEventListener('input', (event) => {
    ...
    if (finalData.length === 0) {
        content.innerHTML = `<div class="alert alert-danger" role="alert">
        Country NOT FOUND or WRONG Input entered!
      </div>`
    }else{
        putData(finalData)
    }
...
getIP().then(ip=>{
    document.querySelector('#ip').textContent = `Your IP-${ip.ip}`
})
